...
#     allow(provisioner).to receive(:create_sandbox).and_return(nil)
#     allow(provisioner).to receive(:cleanup_sandbox).and_return(nil)
...
#       allow(instance.transport).to receive(:connection_options).and_return({ hostname: 'test.host', port: 22, username: 'default_user' })
